...
class Home extends Component {
	...
	handleSubmitProperty = event => {
		...
		API.saveProperty(this.state).then(function(response) {
	...
	handleSubmitOwner = event => {
		event.preventDefault();
		API.saveOwner(this.state).then(function(response) {
			console.log(response);
		});
		this.resetFormOwner();
	};
	...
	handleSubmitTenant = event => {
		event.preventDefault();
		API.saveTenant(this.state).then(function(response) {
			console.log(response);
		});
					...
					<Col size="md-6 sm-12">
						<div className="cardContainer">
							<div className="newPropr card">
								...
								<img
									className="img"
									src="https://image.shutterstock.com/image-photo/stock-image-south-florida-single-260nw-154456190.jpg"
									alt="new property"
								/>
								<NewBtn onClick={() => this.propertyLoadForm()} />
							</div>
							{/* 
							<div className="newPropr card">
								<h6>Owners</h6>
								<img
									className="img"
									src="https://cdn.pixabay.com/photo/2017/05/18/11/04/key-2323278__340.jpg"
									alt="occupancy"
								/>
								<OwnerBtn onClick={() => this.ownerLoadForm()} />
							</div>
						</div>
						...
						<div className="cardContainer">
							<div className="newPropr card">
								...
								<img
									className="img"
									src="https://image.shutterstock.com/image-photo/minature-houses-resting-on-pound-260nw-424211692.jpg"
									alt="rent"
								/>
							...
							<div className="newPropr card">
								...
								<img
									className="img"
									src="https://image.shutterstock.com/image-photo/young-couple-viewing-house-female-260nw-317837381.jpg"
									alt="occupancy"
								/>
								<div>
									<p>Tenanted</p>
								...
								<TenantBtn onClick={() => this.tenantLoadForm()} />
							</div> */}
						</div>
					</Col>
					<Col size="md-6 sm-12">
						{this.state.propertyFormLoaded && (
							<form className="form">
								...
								<Input
									...
									onChange={this.handleInputChange}
									name="beds"
									placeholder="Number beds (Optional)"
								/>
								<Input
									value={this.state.baths}
									onChange={this.handleInputChange}
									name="baths"
									placeholder="Number baths (Optional)"
								/>
								<Input
									value={this.state.sqft}
									onChange={this.handleInputChange}
									name="sqft"
									placeholder="Total sqft (Optional)"
								/>
								<TextArea
									value={this.state.note}
									onChange={this.handleInputChange}
									name="note"
									placeholder="Enter a note for the property (Optional)"
									className="textArea"
								/>
								<FormBtn onClick={this.handleSubmitProperty}>
									Add Property
								</FormBtn>
							</form>
						)}
						{/* {this.state.tenantFormLoaded && (
							<form className="form">
								...
								<Input
									...
									onChange={this.handleInputChange}
									name="homePhone"
									placeholder="Home Phone Nr (Optional)"
								/>
								<Input
